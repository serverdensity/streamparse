...
from .common import (add_ackers, add_config, add_debug, add_environment,
                     add_name, add_options, add_override_name, add_workers,
                     resolve_options)
...
TupleMetadata = namedtuple('TupleMetadata', 'id component stream anchors children values')
...
class FakeStorm(object):
    ...
    def __init__(self, topology_class, env_name, storm_options):
        ...
        for task_id, (component_name, component) in enumerate(self.components.items()):
            ...
            for stream_id, grouping in self.inputs[component_name].items():
                ...
                input_fields = self.outputs[input_component][input_stream].output_fields
                sources[input_component][input_stream] = input_fields
                self.target_components[input_component][input_stream].append(component_name)
    ...
    def emit(self, message, component_name):
        ...
        if component_name in self.spouts:
            ...
            anchors = [expanded_id]
            self.unacked_tuples[expanded_id] = TupleMetadata(tup_id,
                                                             component_name,
                                                             stream_name,
                                                             [component_name],
                                                             set(),
        ...
        if stream_name not in self.outputs[component_name]:
            raise RuntimeError('"{}" is not a defined output stream for "{}". Did '
                               'you forget to set "outputs"?'.format(stream_name,
                                                                     component_name))
        # TODO: direct
        # TODO: need_task_ids
        output_command = {
        ...
        for target_name in self.target_components[component_name][stream_name]:
            ...
            target_component.serializer.send_message(output_command,
                                                     stream=target_component.serializer.input_stream)
            self.unacked_tuples[tup_id] = TupleMetadata(tup_id,
                                                        component_name,
                                                        stream_name,
                                                        anchors,
                                                        set(),
    ...
    def ack(self, tup_id):
        ...
        if num_acked % 20 == 0:
            print('Cumulative count of tuples acked for {}__{}: {}'
                    .format(tup_metadata.component, tup_metadata.stream, num_acked))
        for anchor in tup_metadata.anchors:
            ...
            # Send ack command to spouts
                ...
                spout_obj.serializer.send_message({'command': 'ack',
                                                   'id': tup_id.split('__')[-1]},
    ...
    def fail(self, tup_id):
        ...
        if num_failed % 20 == 0:
            print('Cumulative count of tuples failed for {}__{}: {}'
                    .format(tup_metadata.component, tup_metadata.stream, num_failed))
        ...
        for anchor in tup_metadata.anchors:
            ...
            if anchor == tup_metadata.component:
                ...
                # Stop ignoring spout output from previous sync
                self.ignored_spouts.discard(spout_obj.component_name)
                spout_obj.serializer.send_message({'command': 'fail',
                                                   'id': tup_id},
                                                  stream=spout_obj.serializer.input_stream)
                # Read command and call process
                self.read_and_process(spout_obj)
    ...
    def read_and_process(self, component):
        ...
        if not component.serializer.input_stream:
            return
        component._run()
        message = component.serializer.read_message(stream=component.serializer.output_stream)
        while message is not None:
            command = message['command']
            # Ignore output from ignored spouts
            if component.component_name in self.ignored_spouts:
                print('Ignoring unrequested output from spout {}: {}'
                      .format(component.component_name, message))
            elif command == 'log':
                print(message['msg'])
            # Send output to appropriate downstream bolts
                ...
                stream_name = message.get('stream', 'default')
                num_emitted = self.emitted_counters[component.component_name][stream_name]
                if num_emitted % 20 == 0:
                    print('Cumulative count of tuples emitted for {}__{}: {}'
                          .format(component.component_name, stream_name, num_emitted))
            elif command == 'sync':
        ...
        while now < self.end_time:
            ...
            if now - last_tick > self.tick_freq:
                for bolt_obj in self.bolts.values():
                    tup_id = '{}__tick__{}'.format(bolt_obj.component_name,
                                                   now)
                    ...
                    bolt_obj.serializer.send_message(command,
                                                     stream=bolt_obj.serializer.input_stream)
                    self.read_and_process(bolt_obj)
            ...
            for spout_obj in self.spouts.values():
                ...
                if currently_pending > max_pending:
                    print('More than {} tuples pending for {}: {}'
                          .format(max_pending, spout_obj.component_name, currently_pending))
                    continue
                # Stop ignoring spout output from previous sync
                self.ignored_spouts.discard(spout_obj.component_name)
                # Create next_tuple command
                spout_obj.serializer.send_message({'command': 'next'},
                                                  stream=spout_obj.serializer.input_stream)
                # Read command and call process
                self.read_and_process(spout_obj)
...
def run_python_topology(name=None, env_name=None, running_time=0, options=None,
                        config_file=None, override_name=None):
    ...
    storm_options = resolve_options(options, env_config, topology_class,
                                    override_name, local_only=True)
...
def main(args):
    ...
    run_python_topology(name=args.name, running_time=args.time,
                        options=args.options, env_name=args.environment,
